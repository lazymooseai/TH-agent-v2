...
# --- 3. DATAN HAKU: LENNOT (FINAVIA LIVE XML - VOITTAJA-VERSIO) ---
@st.cache_data(ttl=60) 
def get_flights_live():
    url = "https://apigw.finavia.fi/flights/public/v0/flights"
    api_key = "838062ef175f47708d566bbf5a38a710"
    
    # VIDEOLTA OPITTU TÄSMÄISKU:
    headers = {
        'Cache-Control': 'no-cache',
        'Ocp-Apim-Subscription-Key': api_key,
        'app_key': api_key, # Tämä on se taika-avain, joka avasi oven!
        'Accept': 'application/xml' # Pakotetaan Finavia antamaan se upea XML-rakenne
    }
    ...
    try:
        ...
        # Parsitaan Finavian XML
        root = ET.fromstring(response.content)
        
        # Haetaan vain SAAPUVAT lennot
        saapuvat_lennot = root.findall('.//arr/body/flight')
        
        for flight in saapuvat_lennot:
            # SUODATIN: Otetaan vain Helsinki-Vantaalle (HEL) tulevat koneet
            h_apt = flight.findtext('h_apt', '')
            if h_apt != 'HEL':
                continue
                
            fltnr = flight.findtext('fltnr', '')
            route = flight.findtext('route_n_1', 'Tuntematon')
            actype = flight.findtext('actype', '')
            status_fi = flight.findtext('prt_f', 'Odottaa')
            sdt_str = flight.findtext('sdt', '') # Esim: 2026-03-12T03:20:00Z
            
            # 1. CODESHARE YHDISTÄJÄ (Sama kuin 2018 datassa)
            codeshares = []
            for i in range(1, 7):
                cs = flight.findtext(f'cflight_{i}')
                if cs and cs.strip():
                    codeshares.append(cs.strip())
            
            all_flights = [fltnr] + codeshares
            lento_nimi = " / ".join(all_flights)
            
            # 2. KELLONAIKA JA AIKASUODATIN
            dt_obj = datetime.datetime.min.replace(tzinfo=ZoneInfo("Europe/Helsinki"))
            aika_str = ""
            try:
                dt = datetime.datetime.strptime(sdt_str, "%Y-%m-%dT%H:%M:%SZ")
                dt_obj = dt.replace(tzinfo=datetime.timezone.utc).astimezone(ZoneInfo("Europe/Helsinki"))
                aika_str = dt_obj.strftime("%H:%M")
            except Exception:
                aika_str = sdt_str[11:16] if len(sdt_str) >= 16 else sdt_str
                
            # Hylätään lennot, jotka ovat laskeutuneet yli 1,5 tuntia sitten (eivät enää kiinnosta taksia)
            if dt_obj != datetime.datetime.min and dt_obj < nykyhetki - datetime.timedelta(hours=1.5):
                continue
                
            # 3. LAAJARUNKOTUTKA (Tunnistaa suoraan konetyypistä!)
            laajarunko_koodit = ['359', '350', '333', '330', '340', '788', '789', '777', '77W']
            is_wb = any(wb in actype for wb in laajarunko_koodit)
            tyyppi = f"Laajarunko ✈️ ({actype})" if is_wb else f"Kapearunko ({actype})"
            
            # 4. TAKTINEN TILA
            tila_teksti = status_fi
            if "myöhässä" in status_fi.lower():
                tila_teksti = f"🔴 {status_fi}"
            elif is_wb and "laskeutunut" not in status_fi.lower():
                tila_teksti = f"🔥 Odottaa massapurkua ({status_fi})"
        ...
        # Lajitellaan kaikki päivän lennot aikajärjestykseen ja otetaan 7 tuoreinta/seuraavaa
        lennot_lista = sorted(lennot_lista, key=lambda k: k['datetime_obj'])
        
        if lennot_lista:
            return lennot_lista[:7]
            
        return [{"flight": "Ei tulossa", "origin": "-", "time": "-", "type": "-", "status": "Ei saapuvia lentoja lähiaikoina"}]

    except Exception as e:
        return [{"flight": "API Virhe", "origin": "-", "time": "-", "type": "-", "status": str(e)[:30]}]
...
# LOHKO 3: LENTOKENTTÄ (FINAVIA API LIVE)
