...
def get_flights_live(): 
    ...
    # TAKTIIKKA: "Lekatieto" - Laitetaan avain useaan eri paikkaan
    headers = {
        'Ocp-Apim-Subscription-Key': FINAVIA_API_KEY,
        'Subscription-Key': FINAVIA_API_KEY,
        'Cache-Control': 'no-cache',
        'Accept': 'application/json' 
    }
    
    # Lisätään avain myös URL-parametriksi (tämä selviää usein redirecteistä parhaiten)
    params = {
        'subscription-key': FINAVIA_API_KEY
    }
    ...
    try:
        # allow_redirects=False estää requests-kirjastoa hukkaamasta dataa omineen
        ...
        while response.status_code in (301, 302, 303, 307, 308):
            ...
            # Pakotetaan avaimet (headers ja params) mukaan myös uuteen osoitteeseen!
            response = requests.get(uusi_osoite, headers=headers, params=params, allow_redirects=False, timeout=10)

        if response.status_code == 401:
            virhesyy = response.text
            return [{"flight": "Pääsy evätty", "origin": "Avain hukattiin yhä", "time": "-", "type": "-", "status": "Virhe 401", "debug_json": f"Palvelin vastasi:\n{virhesyy}"}]
