...
def get_flights_live(): 
    # Taktinen muutos: Upotetaan avain suoraan URL-osoitteeseen! Näin se ei katoa uudelleenohjauksissa.
    url = f"https://apigw.finavia.fi/flights/public/v0/flights/arr/HEL?subscription-key={FINAVIA_API_KEY}"
    
    headers = {
        # Lisätään User-Agent, jotta API ei luule meitä haitalliseksi botiksi
    ...
    try:
        # Normaali requests.get - ei estellä uudelleenohjauksia
        response = requests.get(url, headers=headers, timeout=10)

        # Jos saamme 401, pakotetaan Streamlit tyhjentämään välimuisti, jotta emme jää jumiin virheeseen!
