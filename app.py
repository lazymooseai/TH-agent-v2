...
# --- 3. DATAN HAKU: LENNOT (FINAVIA API GATEWAY) ---
@st.cache_data(ttl=60) 
def get_flights():
    url = "https://apigw.finavia.fi/flights/public/v0/flights"
    api_key = "838062ef175f47708d566bbf5a38a710" # SINUN HENKILÖKOHTAINEN AVAIMESI ON NYT TÄSSÄ!
    ...
    try:
        ...
        # Jos avain on väärin, Finavia palauttaa koodin 401
        if response.status_code == 401:
            return [{"flight": "Pääsy evätty", "origin": "API-avain ei kelpaa", "time": "-", "type": "-", "status": "Virhe 401", "debug_json": ""}]
            
        response.raise_for_status()
        data = response.json()
        
        # Otetaan varmuuden vuoksi näyte talteen Röntgeniä varten, 
        # jotta voimme analysoida tarkan rakenteen, jos suora luku epäonnistuu.
        raakadata_nayte = json.dumps(data, indent=2)[:1000]
        
        # --- TEKOÄLYN SOKEA YRITYS PURKAA FINAVIAN JSON ---
        saapuvat = []
        ...
        # Jos yllä oleva yritys ei löytänyt oikeaa laatikkoa, laukaistaan Röntgen
        if not saapuvat:
        ...
        for lento in saapuvat:
            # Otetaan huomioon mahdolliset variaatiot kenttien nimissä JSONissa
            ...
            if "myöhässä" in tila_teksti.lower():
        ...
        return [{"flight": "Luku-virhe", "origin": "-", "time": "-", "type": "-", "status": str(e)[:30], "debug_json": "Virhe yhdistäessä APIin"}]
...
# RÖNTGEN: NÄKYY VAIN JOS TULEE VIRHE "RAKENNETTA EI TUNNISTETTU"
if flights and flights[0].get("debug_json"):
    with st.expander("🔍 TEKOÄLYN RÖNTGEN (Avaa ja ota kuva)"):
        st.write("Näytä minulle tämä JSON-koodinpätkä, niin rakennan datan purun valmiiksi:")
