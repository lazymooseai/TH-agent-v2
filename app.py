...
# --- 2. DATAN HAKU: JUNAT (DIGITRAFFIC API - KORJATTU AIKASUODATIN) ---
...
def get_trains(asema_nimi):
    ...
    # Pyydetään rajapinnalta vain saapuvia (ei menneitä: arrived_trains=0)
    ...
    try:
        ...
        for juna in data:
            ...
            # TAKTINEN SUODATIN 1: Ohitetaan junat, jotka LÄHTEVÄT pk-seudulta
                ...
                if rivi['stationShortCode'] == koodi and rivi['type'] == 'ARRIVAL':
                    ...
                    if aika_raaka:
                        try:
                            ...
                            # TAKTINEN SUODATIN 2: Onko juna jo mennyt? (Sallitaan 2 minuutin heitto)
            ...
            # Jos juna on tulossa ja sitä ei ole peruttu
            if not peruttu and aika_str and aika_obj and not mennyt_juna:
                ...
                junat_lista.append({
                    ...
                    "time": aika_str,
                    "datetime_obj": aika_obj, # Piilotettu arvo lajittelua varten
                    "delay": viive_teksti,
        ...
        # Lajitellaan junat todellisen ajan mukaan (ei aakkosjärjestyksen)
        junat_lista = sorted(junat_lista, key=lambda k: k['datetime_obj'])
        return junat_lista[:5] 

    except Exception as e:
        return [{"train": "API Virhe", "origin": "-", "time": "-", "delay": "Ei", "link": f"https://www.vr.fi/radalla"}]
