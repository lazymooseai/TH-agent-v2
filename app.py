...
# --- 3. DATAN HAKU: LENNOT (SELAIN-SIMULAATIO) ---
@st.cache_data(ttl=60) 
def get_flights_live(): 
    # Hyökätään suoraan julkiselle sivulle täydellä selainnaamioinnilla
    ...
    try:
        ...
        # 1. Yritetään lukea tavallisia HTML-rivejä (jos Finavia sattuu tulostamaan ne koodiin)
        lento_rivit = soup.select('tr.flight-row') or soup.find_all('div', class_=lambda c: c and 'flight' in c.lower())
        
        # 2. Jos sivu on täynnä JavaScriptiä, etsitään "piilotettua" JSON-dataa koodin seasta
        json_data = None
        for script in soup.find_all('script'):
            if script.string and 'window.__INITIAL_STATE__' in script.string:
                try:
                    # Yritetään louhia data ulos JS-muuttujasta
        ...
        # Röntgen-varoituksen valmistelu
        raakadata_nayte = response.text[:1000] if not json_data else json.dumps(json_data, indent=2)[:1000]

        # Jos löysimme datan JSON-muodossa
        if json_data:
            # Tässä joudumme arpomaan rakenteen, otetaan talteen Röntgenillä jos pieleen menee
            saapuvat = []
            try:
                # Arvataan yleisiä rakenteita sivuston state-objektissa
                saapuvat = json_data.get('flights', {}).get('arr', [])
            except:
                pass
            ...
            if saapuvat:
                ...
                for lento in saapuvat:
                    fltnr = lento.get('fltnr', '??')
                    kohde = lento.get('route_n_1', 'Tuntematon')
                    aika_raaka = lento.get('sdt', '')
                    actype = lento.get('actype', '')
                    status = lento.get('prt_f', 'Odottaa')
                    ...
                    tyyppi = f"Laajarunko ✈️" if is_wb else f"Kapearunko"
                    ...
                    lennot_lista.append({"flight": fltnr, "origin": kohde, "time": aika_str, "type": tyyppi, "status": tila_teksti, "debug_json": ""})
                
                if lennot_lista: return lennot_lista[:7]
                
        # Jos kumpikaan (HTML tai JSON) ei toiminut
        return [{"flight": "HTML Sivu ladattu", "origin": "Data piilotettu JS:n taakse", "time": "-", "type": "-", "status": "Tarvitaan Röntgeniä", "debug_json": raakadata_nayte}]

    except Exception as e:
        return [{"flight": "Yhteysvirhe sivuun", "origin": "-", "time": "-", "type": "-", "status": str(e)[:30], "debug_json": ""}]
...
# LOHKO 3: LENTOKENTTÄ (SELAIN-SIMULAATIO)
st.markdown("<h3 style='color:#e0e0e0; margin-top:30px; font-size: 26px;'>✈️ LENTOKENTTÄ (Julkinen Data)</h3>", unsafe_allow_html=True)
flights = get_flights_live()
flight_html = f"<div style='margin-bottom: 15px; color: #a3c2a3;'>Näkymä: <b>Taktiset poiminnat</b></div>"

for f in flights:
    status_color = "#ff4b4b" if "🔥" in f['status'] or "🔴" in f['status'] or "Virhe" in f['status'] or "Tarvitaan" in f['status'] else "#a3c2a3"
...
if flights and flights[0].get("debug_json"):
    with st.expander("🔍 TEKOÄLYN RÖNTGEN (Avaa ja ota kuva)"):
        st.write("Näytä minulle tämä HTML/JSON koodinpätkä:")
        st.code(flights[0]["debug_json"], language='html')
