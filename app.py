...
import datetime
import urllib.request # UUSI: Raaka, matalan tason HTTP-kirjasto
import urllib.error
import json
from bs4 import BeautifulSoup
from zoneinfo import ZoneInfo
import requests # Säilytetään satamaa ja junia varten
...
# --- 3. DATAN HAKU: LENNOT (FINAVIA RAAKA HTTP) ---
@st.cache_data(ttl=60) 
def get_flights_raw(): # Nimi vaihdettu taas cachen takia
    url = "https://apigw.finavia.fi/flights/public/v0/flights"
    api_key = "838062ef175f47708d566bbf5a38a710" 
    
    # RAAKA HTTP PYYNTÖ: Tämä ohittaa pythonin "requests" kirjaston omituisuudet
    ...
    try:
        with urllib.request.urlopen(req, timeout=10) as response:
            ...
            if not saapuvat:
                return [{"flight": "Ei dataa", "origin": "-", "time": "-", "type": "-", "status": "Lista tyhjä"}]
        ...
        if e.code == 401:
            return [{"flight": "Pääsy evätty", "origin": "Avain virheellinen", "time": "-", "type": "-", "status": "Virhe 401"}]
        return [{"flight": "Palvelinvirhe", "origin": "-", "time": "-", "type": "-", "status": f"Virhe {e.code}"}]
    except Exception as e:
...
flights = get_flights_raw() # UUSI FUNKTIO!
