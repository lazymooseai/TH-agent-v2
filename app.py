...
# --- 3. DATAN HAKU: LENNOT (FINAVIA PANSSAROITU ISTUNTO) ---
@st.cache_data(ttl=60) 
def get_flights_live(): 
    url = "https://apigw.finavia.fi/flights/public/v0/flights"
    api_key = "838062ef175f47708d566bbf5a38a710" 
    
    # LUODAAN ISTUNTO (Session). Tämä pitää API-avaimen muistissa vaikka Finavia ohjaisi meidät eteenpäin!
    istunto = requests.Session()
    istunto.headers.update({
    ...
    try:
        response = istunto.get(url, timeout=10)
        
        if response.status_code == 401:
            virhesyy = response.text
            return [{"flight": "Pääsy evätty", "origin": "Avain puuttuu ohjauksen jälkeen", "time": "-", "type": "-", "status": "Virhe 401", "debug_json": f"Vastaus:\n{virhesyy}"}]
...
# RÖNTGEN: NÄKYY VAIN JOS TULEE VIRHE "RAKENNETTA EI TUNNISTETTU" TAI "401"
if flights and flights[0].get("debug_json"):
    with st.expander("🔍 TEKOÄLYN RÖNTGEN (Avaa ja ota kuva)"):
        st.write("Näytä minulle tämä tekstilaatikko, niin ratkaisen ongelman heti:")
