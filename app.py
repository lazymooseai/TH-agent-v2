...
# --- 2. DATAN HAKU: JUNAT (DIGITRAFFIC API) ---
...
# --- 3. DATAN HAKU: LENNOT (LIVE SCRAPER & TEKOÄLY) ---
@st.cache_data(ttl=120) 
def get_flights():
    url = "https://www.airport-helsinki.com/arrivals.php"
    headers = {'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64)'}
    
    # Laajarunkokohteiden tunnistus (lisätty Lontoo)
    laajarunko_kohteet = ['tokyo', 'osaka', 'seoul', 'shanghai', 'hong kong', 'singapore', 
                          'bangkok', 'phuket', 'doha', 'dubai', 'new york', 'los angeles', 
                          'dallas', 'miami', 'delhi', 'peking', 'beijing', 'haneda', 'narita', 'london', 'lontoo']
    
    lennot_lista = []
    naellyt_lennot = set() # SUODATIN: Tähän tallennetaan jo nähdyt lennot
    ...
    try:
        ...
        for rivi in rivit:
            ...
            if len(solut) >= 5:
                aika = solut[0].text.strip()
                lento = solut[1].text.strip()
                kohde = solut[2].text.strip()
                yhtio = solut[3].text.strip()
                status = solut[4].text.strip()
                
                # Varmistetaan, että kyseessä on oikea datarivi
                if ":" in aika and lento:
                    
                    # TAKTINEN SUODATIN: Luodaan yksilöllinen tunniste (esim. "12:05-Istanbul")
                    tunniste = f"{aika}-{kohde}"
                    
                    # Jos tätä aika+kohde -yhdistelmää ei ole vielä nähty:
                    if tunniste not in naellyt_lennot:
                        naellyt_lennot.add(tunniste) # Lisätään muistiin
                        
                        is_laajarunko = any(kaupunki in kohde.lower() for kaupunki in laajarunko_kohteet)
                        tyyppi = "Laajarunko ✈️" if is_laajarunko else "Kapearunko"
                        tila_teksti = status
                        
                        if "delayed" in status.lower() or "myöhässä" in status.lower():
                            tila_teksti = f"🔴 Myöhässä ({status})"
                        elif is_laajarunko and "arrived" not in status.lower() and "landed" not in status.lower():
                            tila_teksti = f"🔥 Odottaa massapurkua ({status})"
                            
                        lennot_lista.append({
                            "flight": lento,
                            "origin": f"{kohde} ({yhtio})",
                            "time": aika,
                            "type": tyyppi,
                            "status": tila_teksti
                        })
        ...
        if lennot_lista:
            # Suodatetaan pois jo laskeutuneet tai terminaalissa olevat lennot
            tulevat = [l for l in lennot_lista if "arrived" not in l['status'].lower() and "landed" not in l['status'].lower()]
            return tulevat[:7] # Palautetaan taktisesti 7 seuraavaa lentokonetta
            
        return [{"flight": "Ei dataa", "origin": "-", "time": "-", "type": "-", "status": "Taulukkoa ei löytynyt"}]
        
    except Exception as e:
        return [{"flight": "Yhteysvirhe", "origin": "-", "time": "-", "type": "-", "status": str(e)[:30]}]
...
# LOHKO 3: LENTOKENTTÄ (LIVE)
st.markdown("<h3 style='color:#e0e0e0; margin-top:30px; font-size: 26px;'>✈️ LENTOKENTTÄ (HELSINKI-VANTAA LIVE)</h3>", unsafe_allow_html=True)
flights = get_flights()
flight_html = f"<div style='margin-bottom: 15px; color: #a3c2a3;'>Näkymä: <b>Taktiset poiminnat ({len(flights)} kpl)</b></div>"
for f in flights:
    status_color = "#ff4b4b" if "🔥" in f['status'] or "🔴" in f['status'] else "#a3c2a3"
