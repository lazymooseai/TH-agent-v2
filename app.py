...
# Tämä koodinpätkä pakottaa selaimen päivittämään sivun minuutin välein.
# Näin kello ja junien myöhästymiset ovat aina aidosti reaaliajassa ilman napin painallusta.
components.html(
    """
    <script>
    setTimeout(function(){
...
@st.cache_data(ttl=50) # Välimuisti pidetään alle 60s, jotta auto-refresh hakee aina uuden datan
def get_trains(asema_nimi):
    asema_koodit = {'Helsinki': 'HKI', 'Pasila': 'PSL', 'Tikkurila': 'TKL'}
    koodi = asema_koodit.get(asema_nimi, 'HKI')
    
    # Haetaan 30 junaa, jotta suodatuksen jälkeenkin jää tarpeeksi näytettävää
    url = f"https://rata.digitraffic.fi/api/v1/live-trains/station/{koodi}?arriving_trains=30&train_categories=Long-distance"
    
    kaupungit = {'ROV': 'Rovaniemi', 'OUL': 'Oulu', 'TPE': 'Tampere', 'TKU': 'Turku', 
                 'KAI': 'Kajaani', 'JNS': 'Joensuu', 'YV': 'Ylivieska', 'VAA': 'Vaasa', 'JY': 'Jyväskylä', 'KOP': 'Kuopio', 'POR': 'Pori'}
    ...
    try:
        ...
        for juna in data:
            ...
            # TAKTINEN SUODATIN: Ohitetaan kaikki junat, jotka ovat lähtöisin Helsingistä tai Pasilasta.
            # Haluamme nähdä vain junat, jotka TUOVAT asiakkaita.
                ...
                if rivi['stationShortCode'] == koodi and rivi['type'] == 'ARRIVAL':
                    ...
                    if aika_raaka:
                        try:
                            ...
                            aika_obj = aika_obj + datetime.timedelta(hours=2) # Suomen aika (Talviaika)
        ...
        return junat_lista[:5] # Näytetään 5 seuraavaa oikeaa tulijaa
...
st.markdown(f"<div class='taksi-card'>{train_html}<a href='https://www.vr.fi/radalla' class='taksi-link' target='_blank'>Avaa VR Radalla ➔</a></div>", unsafe_allow_html=True)
