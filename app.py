...
# --- 3. DATAN HAKU: LENNOT (LÄPIMURTO) ---
@st.cache_data(ttl=60) 
def get_flights_live(): 
    # Käytetään varmuuden vuoksi suoraa HEL-saapuvien osoitetta, jos se on APIn vaatimus
    url = "https://apigw.finavia.fi/flights/public/v0/flights"
    api_key = "838062ef175f47708d566bbf5a38a710" 
    
    # TAKTIIKKA: Avain on pakotettu URL-osoitteeseen parametrina. 
    # Tämä selviää hengissä kaikista uudelleenohjauksista (redirect).
    params = {
    ...
    try:
        # Laitetaan ohjaukset päälle, mutta avain on turvassa params-osiossa
        response = requests.get(url, headers=headers, params=params, allow_redirects=True, timeout=10)
        
        if response.status_code == 401:
            virhe = response.text
            return [{"flight": "Pääsy evätty", "origin": "URL-avain hylättiin", "time": "-", "type": "-", "status": "Virhe 401", "debug_json": virhe}]
        ...
        for lento in saapuvat:
            fltnr = lento.get('fltnr', lento.get('flightNumber', lento.get('id', '??')))
            kohde = lento.get('route_n_1', lento.get('airport', lento.get('origin', 'Tuntematon')))
            aika_raaka = lento.get('sdt', lento.get('scheduledTime', lento.get('time', '')))
        ...
        if lennot_lista:
            # Lajitellaan ajan mukaan ja palautetaan 7 seuraavaa
            lennot_lista = sorted(lennot_lista, key=lambda k: k['time'])
            return lennot_lista[:7]
            
        return [{"flight": "Lista tyhjä", "origin": "-", "time": "-", "type": "-", "status": "Ei saapuvia lentoja", "debug_json": raakadata_nayte}]
...
if flights and flights[0].get("debug_json"):
    with st.expander("🔍 TEKOÄLYN RÖNTGEN (Avaa ja ota kuva)"):
        st.write("Jos näet tämän, onnistuimme murtamaan oven! Ota kuva tästä jsonista, niin säädän listan nätiksi:")
